# Authors and Citation

## Authors

- **Pau Satorra**. Author, maintainer.
  [](https://orcid.org/0000-0002-8144-4089)

- **Marcos Quijal-Zamorano**. Author.

- **Joan Ballester**. Author.

- **Cristian Tebé**. Contributor.

- **Miguel A. Martínez-Beneito**. Author, thesis advisor.

- **Marc Marí-Dell'Olmo**. Author, thesis advisor.

## Citation

Source:
[`DESCRIPTION`](https://github.com/pasahe/bdlnm/blob/main/DESCRIPTION)

Satorra P, Quijal-Zamorano M, Ballester J, Martínez-Beneito M,
Marí-Dell'Olmo M (2026). *bdlnm: Bayesian Distributed Lag Non-Linear
Models (B-DLNM)*. R package version 0.1.1,
<https://github.com/pasahe/bdlnm>.

    @Manual{,
      title = {bdlnm: Bayesian Distributed Lag Non-Linear Models (B-DLNM)},
      author = {Pau Satorra and Marcos Quijal-Zamorano and Joan Ballester and Miguel A. Martínez-Beneito and Marc Marí-Dell'Olmo},
      year = {2026},
      note = {R package version 0.1.1},
      url = {https://github.com/pasahe/bdlnm},
    }
